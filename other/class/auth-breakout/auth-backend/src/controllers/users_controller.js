...
const loginUser = async (req, res, next) => {
  ...
  if (!username || !password) return next({error: 'Missing username or password', status: 400})
  const user = await model.getUserByUsername(username.toLowerCase());
  ...
  if (isValid) {
    ...
    return next({error: 'Username or password invalid', status: 400});
  }
